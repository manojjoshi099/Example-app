        ...
        <div class="card-body">
            <form action="{{ isset($category) ? route('categories.update', $category->id) : route('categories.store') }}" method="POST">
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="numeric" name="price" class="form-control" required value="{{ $category->price ?? old('price') }}">
                </div>
                ...
                <tbody>
                    @forelse($categories as $key => $cat)
                        <tr>
                            ...
                            <td>{{ $cat->price }}</td>
                            <td>
                                <a href="{{ route('categories.edit', $cat->id) }}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                <form action="{{ route('categories.destroy', $cat->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirm('Are you sure?')" class="btn btn-sm btn-danger"><i class="fas fa-remove"></i> Delete</button>
                                </form>
                            </td>
                        </tr>
                    @empty
